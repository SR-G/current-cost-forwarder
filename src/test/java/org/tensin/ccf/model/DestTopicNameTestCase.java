...
public class DestTopicNameTestCase extends AbstractReaderTestCase {
    ...
    public void testTopicName() throws CCFException {
        try {
            ...
            Assert.assertEquals("/metrics/current-cost/0/watts", w.enhanceTopicWithInternalValues("/metrics/current-cost/${sensor}"));
            Assert.assertEquals("/metrics/current-cost/0/watts", w.enhanceTopicWithInternalValues("/metrics/current-cost/${sensor}/"));
            Assert.assertEquals("/metrics/current-cost/00077/watts", w.enhanceTopicWithInternalValues("/metrics/current-cost/${id}"));
            Assert.assertEquals("/metrics/current-cost/00077/watts", w.enhanceTopicWithInternalValues("/metrics/current-cost/${id}/"));
        } catch (Exception e) {
